...
class AddUserRequest(BaseRequest):
    ...
    def __init__(self, project: str, service: str,
        ...
        super().__init__(project, service, 'add_user', qos,
                         serializer, deserializer, request_queue, timeout)

    def send_request(self, userName: str,
                     password: str, access: str, **kwargs) -> StandardResponse:
        self._request = AddUserMessage(
            userName=userName,
            password=password,
            access=access
        )
